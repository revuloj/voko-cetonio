...
agordo :-
  ...
  assert(user:file_search_path(tekstoj,TekstojPado)),
  agordo:get_config(verkoj,VerkojPado),
  assert(user:file_search_path(verkoj,VerkojPado)).
...
vrk(ra,rabistoj,verkoj('rabistoj.xml')).
...
verkaro(klasikaj,[pv,mt,nt,fb,gf,fr,fe,ee,ik,hm,pa,po,bu,ma,ha,ba,op,ra,rv,vz,if,lr,fk,dl]).
...
% Rabistoj
verko_teksto(ra:No,Titolo,File) :- !,
   teix_teksto(ra:No,Titolo,File,[act]).
