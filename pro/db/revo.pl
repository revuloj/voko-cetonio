...
search_eo(Kion,Row) :-
    check_search(Kion),
    format(atom(Query),'select kap,num,art,mrk from _kap where kap like ''~w%'' collate nocase order by kap collate nocase, dis',[Kion]),
...
search_eo_limit(Kion,Row,Limit) :-
    check_search(Kion),
    format(atom(Query),
	   'select kap,num,art,mrk from _kap where kap like ''~w%'' order by kap collate nocase, dis limit ''~d''',[Kion,Limit]),
    debug(sqlrevo,'query=~q',[Query]),
    sqlite_query(revodb,Query,Row).
