...
initial_context(
	...
	( member( extended_characters(false), Controls ) ->
		Entities = Empty
	; otherwise ->
	    % hack to provide an entity list, e.g. from external DTD  
	  (
	   member( entities(AddEntities), Controls ) -> 
             (
	        extended_character_entities(ExtEntities),
	        append(ExtEntities,AddEntities,Entities_),
	        keysort(Entities_,Entities)
             )
	  ; otherwise ->
